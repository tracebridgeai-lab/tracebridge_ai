...
async function runExhaustiveTests() {
    console.clear();
    console.log(`\n${C.bgBlue}${C.bold} TRACEBRIDGE AI : EXHAUSTIVE FDA VERTICAL EVALUATION ${C.rst}`);
    console.log(`${C.dim}Cross-validating Class I, Class III, SaMD, and Hardware extrema logic...\n${C.rst}`);

    const filenames = [
        "TEST_01_ISO_13485_QMS_Pass.txt",
        "TEST_02_Cybersecurity_SBOM_Fail.txt",
        "TEST_03_IEC_62304_Architecture_Pass.txt",
        "TEST_04_ISO_14971_Risk_Matrix_Fail.txt",
        "TEST_05_ISO_10993_Biocompatibility_Implant.txt",
        "TEST_06_SaMD_App_Architecture.txt",
        "TEST_07_Class1_Tongue_Depressor.txt",
        "TEST_08_IEC_62366_Human_Factors_Fail.txt"
    ];
    ...
    try {
        console.log(`${C.cya}▶ Mounting ${files.length} FDA Document Vectors...${C.rst}`);
        console.log(`${C.cya}▶ Dispatching cognitive evaluation against 8 device classes...${C.rst}\n`);
        
        const startTime = Date.now();
        const results = await queryGeminiRESTArray(files, EXHAUSTIVE_RULES);
        ...
        results.forEach(res => {
            ...
            const isTrap = res.ruleId.includes("Fail") || res.ruleId.includes("human_factors") || res.ruleId.includes("cyber") || res.ruleId.includes("risk");
            const statusStr = isPass ? `${C.grn}${C.bold}[✓ CONFIRMED]${C.rst}` : `${C.red}${C.bold}[✕ CRITICAL GAP]${C.rst}`;
            const analyticalStr = isPass && isTrap ? `${C.red}FALSE POSITIVE FAILURE${C.rst}` : (isPass ? "True Positive (Golden)" : "True Negative (Trap Triggered)");

            console.log(`\n${C.bold}RULE:${C.rst}      ${C.blu}${rule.standard} § ${rule.section}${C.rst}`);
            console.log(`${C.bold}TEST VECTOR:${C.rst} ${analyticalStr}`);
        ...
        console.log(`\n${C.grn}${C.bold}✓ EXHAUSTIVE SUITE NOMINAL. ALL EXTREMA BOUNDARIES VALIDATED.${C.rst}\n`);
